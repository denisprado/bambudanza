              ...
              <Card as='article'
                ...
                <AspectRatio ratio={4 / 3} bg={'gray'}>
                  <Link href={clase.fields.slug}>
                    <Box sx={{ filter: 'grayscale(1)', backgroundSize: 'cover', height: '100%', backgroundImage: `url('${clase.frontmatter.featuredimage.childImageSharp.fluid.src}')`, backgroundPosition: 'top center' }} />
                  </Link>
                </AspectRatio>

                <Box>
                  <Flex mt={2} sx={{ justifyContent: "space-between", alignItems: 'flex-start' }}>
                    <Link href={clase.fields.slug} sx={{ flex: 1 }}>
                      ...
                      <Text sx={{
                        fontSize: 1,
                        fontStyle: 'italic',
                      }}>{clase.frontmatter.estilo}</Text>
                    </Link>
                    <Badge bg={'primary'} px={2} sx={{ marginLeft: 'auto' }}>{clase.frontmatter.tipo}</Badge>
                  </Flex>

                  <Flex mt={1} sx={{ justifyContent: 'flex-start', alignItems: 'center' }}>
                    <Flex>
                      {clase.frontmatter.nivel.map(n =>
                        <IconsNivel key={n} mr={1} nivel={n} />
                      )
                      }
                    </Flex>
                    <Text mx={2}>{clase.frontmatter.dias}</Text>
                    <Text mr={2}>»</Text><Text>{clase.frontmatter.horarios}</Text>
                  </Flex>
  ...
  <StaticQuery
    query={graphql`
      query OnlineRollQuery {
        allMarkdownRemark(sort: {order: ASC, fields: [frontmatter___title]}, filter: {frontmatter: {templateKey: {eq: "online-post"}}}) {
          edges {
            node {
